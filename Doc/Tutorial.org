...
*** Deferred Replacement
    ...
    =MatchRes m= should not be modified between the calls as it stores the start
...
*** Examples of Pattern Matching
    ...
    #+begin_src c++
      Match("xy[ab{cd}]", balancedString % cout & Fail());
    #+end_src
    ...
    deferred call to =GtS.getBool()= tests if the string we assigned to =cur= is
    the longest so far. If not, then failure is signalled, and we seek
    alternatives (this means that =BreakX= will extend and look for the next
    digit string).  If the call to =GtS.getBool()= succeeds then the matched
    string is assigned as the largest string so far into =max= and its location
    is saved in =loc=. Finally =Fail= forces the match to fail and seek
